...
- (id)init;
{
    ...
    GPUImageBoxBlurFilter *boxBlurFilter = [[[GPUImageBoxBlurFilter alloc] init] autorelease];
    [self addFilter:boxBlurFilter];
    
    // Third pass: compare the blurred background luminance to the local value
    GPUImageFilter *adaptiveThresholdFilter = [[[GPUImageFilter alloc] initWithFragmentShaderFromString:kGPUImageAdaptiveThresholdFragmentShaderString] autorelease];
