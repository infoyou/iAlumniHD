...
- (NSData *)syncGet:(NSString *)urlStr {
	...
	if (![AppManager instance].networkStable) {
        ...
        [WXWUIUtils showNotificationOnTopWithMsg:LocaleStringForKey(NSConnectionErrorMsg, nil)
                                      msgType:ERROR_TY
                           belowNavigationBar:YES];
        return nil;
    }
...
- (NSData *)syncPost:(NSString *)aUrl paramDic:(NSDictionary *)paramDic {
	...
	NSString *contentType = [NSString stringWithFormat:@"multipart/form-data; boundary=%@", IALUMNIHD_FORM_BOUNDARY];
