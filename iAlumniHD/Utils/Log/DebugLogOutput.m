...
- (void)saveLogContentToFile:(NSString *)logFileName
    ...
    NSLog(@"-------- User ID: %@ | Device: %@ | iOS Version: %.1f | APP Version: %@ --------\nFile:%@ Line:%d %@",
...
- (NSDirectoryEnumerator *)checkLogFolder {
	...
	NSString *docDirectory = [CommonUtils documentsDirectory];
...
- (BOOL)saveContentIntoFile:(NSString *)logFile
    ...
    if (NSOrderedSame == res) {
        ...
        if (_logType == CRASH_LOG_TY || _logType == ERROR_LOG_TY) {
            
            // save into zip file
            [CommonUtils saveToZipFile:[LOG_FOLDER stringByAppendingPathComponent:logFile]
                         logFolderPath:LOG_FOLDER
...
- (void)createLogFileAndSave:(NSString *)logPrefix
    ...
    NSString *userId = [AppManager instance].userId;
    ...
    if (_logType == CRASH_LOG_TY) {
        ...
        // save into zip file
        [CommonUtils saveToZipFile:[LOG_FOLDER stringByAppendingPathComponent:logFileName]
                     logFolderPath:LOG_FOLDER
               zipNoSuffixFileName:self.noSuffixCrashFileName];
...
- (void)outputCrash:(char *)fileName
    ...
    NSString *currentHourTime = [CommonUtils currentHourTime];
