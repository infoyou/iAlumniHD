...
#define PORTRAIT_WIDTH		SCREEN_HEIGHT
#define PORTRAIT_HEIGHT		UI_MODAL_PAGE_SHEET_WIDTH

#define LANDSCAPE_WIDTH		UI_MODAL_PAGE_SHEET_WIDTH
#define LANDSCAPE_HEIGHT	SCREEN_HEIGHT
...
#define PORTRAIT_W_H_RATIO	(LIST_WIDTH/HEIGHT) 
...
- (id)initWithImage:(UIImage *)anImage {
    ...
    self = [super initWithMOC:nil frame:CGRectMake(0, 0, UI_MODAL_PAGE_SHEET_WIDTH, SCREEN_HEIGHT)];
    if (self) {
		self.image = anImage;
	}
	return self;
...
- (void)fetchImage:(NSString *)imageUrl forceNew:(BOOL)forceNew {
    ...
    [[[AppManager instance] imageCache] fetchImage:imageUrl caller:self forceNew:forceNew];
}
...
- (void)imageFetchFailed:(NSError *)error url:(NSString *)url {
    ...
    [WXWUIUtils showNotificationOnTopWithMsg:LocaleStringForKey(NSLoadImageFailedMsg, nil)
                                  msgType:ERROR_TY
...
- (void)initScrollView {
    ...
    imgScrollView.frame = CGRectMake(0, 0, self.frame.size.width, self.frame.size.height);
...
//    [WXWUIUtils showNotificationOnTopWithMsg:LocaleStringForKey(NSSaveImageSuccessMsg, nil)
//                                  msgType:SUCCESS_TY
//                       belowNavigationBar:NO];

    [WXWUIUtils showNotificationWithMsg:LocaleStringForKey(NSSaveImageSuccessMsg, nil)
                             msgType:SUCCESS_TY
...
- (void)addBackBtn
{
    UISegmentedControl *segmentedControl = [[UISegmentedControl alloc] initWithItems:[NSArray arrayWithObject:LocaleStringForKey(NSBackTitle, nil)]];
...
- (void)addSaveBtn
{
    UIBarButtonItem *saveBtn = [[[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemSave
                                                                                target:self
                                                                                action:@selector(saveImage)] autorelease];
