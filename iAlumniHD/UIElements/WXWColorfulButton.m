...
#define ENABLED_KEYPATH   @"enabled"
...
@implementation UIButton(WXWColorfulButton)
...
@implementation UIColor(WXWColorfulButton)
...
@implementation WXWColorfulButton
...
- (void) setupSelf {
  ...
  [self addObserver:self forKeyPath:HIGHLIGHT_KEYPATH options:0 context:nil];
  [self addObserver:self forKeyPath:ENABLED_KEYPATH options:0 context:nil];
}
...
-(void) dealloc {
  ...
  [self removeObserver:self forKeyPath:ENABLED_KEYPATH];
...
{
  if ([keyPath isEqualToString:HIGHLIGHT_KEYPATH] || [keyPath isEqualToString:ENABLED_KEYPATH]) {
...
- (void) strokeButton:(CGContextRef)context
                color:(UIColor *)color
...
- (void)setNavigationButtonWithColor:(UIColor*)tintcolor borderColor:(UIColor *)borderColor {
	self.tintColor = tintcolor;
  ...
  if (borderColor) {
	...
	[self setGradientType:kUIGlossyButtonGradientTypeLinearGlossyStandard];
	[self.titleLabel setFont:BOLD_FONT(14)];
	[self useWhiteLabel: NO];
...
@implementation ECStandardButton
...
- (id)initWithFrame:(CGRect)frame
  ...
  if (self) {
    ...
    [self setNavigationButtonWithColor:tintColor borderColor:borderColor];
    [self setTitle:title forState:UIControlStateNormal];
    [self addTarget:target action:action forControlEvents:UIControlEventTouchUpInside];
