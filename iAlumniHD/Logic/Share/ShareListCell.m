...
imageClickableDelegate:(id<ECClickableElementDelegate>)imageClickableDelegate
    ...
    if (self) {
        ...
        _distanceLabel = [[[WXWLabel alloc] initWithFrame:CGRectZero
                                               textColor:BASE_INFO_COLOR
...
#pragma mark - draw SharePost
- (void)showImages:(SharePost *)post {
...
- (void)drawPost:(SharePost *)post MOC:(NSManagedObjectContext *)MOC {
    ...
    if (post.locationAttached.boolValue) {
        ...
        float distance = 0l;
        if (post.distance.floatValue >= 0)
            distance = post.distance.floatValue;
        
        _distanceLabel.hidden = NO;
        _distanceLabel.text = [NSString stringWithFormat:@"%0.2f %@",
                               distance, LocaleStringForKey(NSKMTitle, nil)];
...
- (void)hideLabelShadow {
    ...
    _tagsView.nameLabel.shadowOffset = CGSizeMake(0, 0);
    _tagsView.nameLabel.shadowColor = TRANSPARENT_COLOR;
}
...
- (void)setHighlighted:(BOOL)highlighted animated:(BOOL)animated {
    ...
    if (highlighted) {
        ...
        _tagsView.nameLabel.shadowOffset = CGSizeMake(0, 1.0f);
        _tagsView.nameLabel.shadowColor = [UIColor whiteColor];
...
#pragma mark - browser SharePost image
