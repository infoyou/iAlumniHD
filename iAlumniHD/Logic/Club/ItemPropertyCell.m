...
- (void)selectTag:(Tag *)tag {
    ...
    [CoreDataUtils saveMOCChange:tag.managedObjectContext];
    
    switch (_tagType) {
        case THING_TY:
        {
            [AppManager instance].defaultThing = tag.tagName;
        ...
        case PLACE_TY:
        {
            [AppManager instance].defaultPlace = tag.tagName;
...
- (void)selectShareTag:(Tag *)tag {
    ...
    if (tag.tagId.longLongValue == TAG_ALL_ID && tag.selected.boolValue) {
        ...
        for (Tag *otherTag in tags) {
            otherTag.selected = [NSNumber numberWithBool:NO];
        }
        ...
        Tag *allTag = (Tag *)[CoreDataUtils fetchObjectFromMOC:_MOC
                                                    entityName:@"Tag"
                                                     predicate:predicate];
        allTag.selected = [NSNumber numberWithBool:NO];
    }
    
    // if no selected tag, then "All" tag should be selected, which means there is one tag should be selected at least
    NSInteger selectedTagCount = [CoreDataUtils objectCountsFromMOC:_MOC entityName:@"Tag" predicate:SELECTED_PREDICATE];
...
- (void)selectDistance:(Distance *)distance {
    ...
    NSArray *distances = [CoreDataUtils fetchObjectsFromMOC:distance.managedObjectContext
                                                 entityName:@"Distance"
                                                  predicate:predicate];
    for (Distance *obj in distances) {
        obj.selected = [NSNumber numberWithBool:NO];
    }
            ...
            tagType:(TagType)tagType
{
    ...
    if (self) {
        ...
        switch (_itemType) {
            ...
            case TWO_TY:
            {
                [self initButton:&_leftButton frame:CGRectMake(0, 0, _twoButtonWidth, ITEM_PROPERTY_CELL_HEIGHT)];
                [_leftButton addTarget:self action:@selector(selectLeftItem:) forControlEvents:UIControlEventTouchUpInside];
                
                [self initButton:&_rightButton frame:CGRectMake(_twoButtonWidth + 2, 0, _twoButtonWidth + 1, ITEM_PROPERTY_CELL_HEIGHT)];
...
{
    if (isFromComposer) {
        _twoButtonWidth = 269.f;
    } else {
        _twoButtonWidth = TWO_BUTTON_WIDTH;
    }
    
    return [self initWithStyle:style reuseIdentifier:reuseIdentifier editorDelegate:editorDelegate itemType:itemType MOC:MOC sharingFilterType:sharingFilterType];
}
