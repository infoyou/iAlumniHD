...
- (void)initEventBottom {
    ...
    WXWLabel *signUpNumLabel = [[[WXWLabel alloc] initWithFrame:CGRectMake(ARROW_X - 64.f, 11.f, 60.f, 20.f) textColor:[UIColor whiteColor] shadowColor:[UIColor grayColor]] autorelease];
    signUpNumLabel.font = FONT(22);
    signUpNumLabel.text = [NSString stringWithFormat:@"%@", self.event.signupCount];
    signUpNumLabel.textAlignment = NSTextAlignmentCenter;
    [_eventSignBut addSubview:signUpNumLabel];
    
    UIImageView *signUpArrowView = [[[UIImageView alloc] initWithImage:[UIImage imageNamed:@"eventLightArrow.png"]] autorelease];
    ...
    _eventCheckinBut = [[[WXWImageButton alloc]
                         initImageButtonWithFrame:CGRectMake(LIST_WIDTH/2+MARGIN, _postImgButton.frame.origin.y + POST_H + 2 * MARGIN, ACTION_BUTTON_WIDTH, ACTION_BUTTON_HEIGHT)
                         target:self
                         action:@selector(goCheckInList:)
                         title:LocaleStringForKey(NSAttendTitle, nil)
                         image:nil
                         backImgName:@"eventCheckin.png"
                         selBackImgName:nil
                         titleFont:FONT(16.f)
                         titleColor:[UIColor whiteColor]
                         titleShadowColor:TRANSPARENT_COLOR
                         ...
                         imageEdgeInsert:ZERO_EDGE
                         titleEdgeInsert:UIEdgeInsetsMake(0, -75, 0, 0)] autorelease];
    [WXWUIUtils addShadowForButton:_eventCheckinBut];
    [self addSubview:_eventCheckinBut];
    
    WXWLabel *checkInNumLabel = [[[WXWLabel alloc] initWithFrame:CGRectMake(ARROW_X - 64.f, 11.f, 60.f, 20.f) textColor:[UIColor whiteColor] shadowColor:[UIColor grayColor]] autorelease];
    ...
    UIImageView *checkInArrowView = [[[UIImageView alloc] initWithImage:[UIImage imageNamed:@"eventLightArrow.png"]] autorelease];
    ...
    /*
     ...
     WXWImageButton *eventActionBut = [[[WXWImageButton alloc]
     initImageButtonWithFrame:CGRectMake(2 * MARGIN, _eventSignBut.frame.origin.y + 40.f + 2 * MARGIN, 300.f, 32.f)
     target:self
     action:@selector(doSignUp:)
     title:self.event.actionStr
     image:nil
     backImgName:@"eventAction.png"
     selBackImgName:nil
     titleFont:FONT(16.f)
     titleColor:[UIColor whiteColor]
     titleShadowColor:TRANSPARENT_COLOR
     ...
     [self addSubview:eventActionBut];
     
     CGSize actionSize = [self.event.actionStr sizeWithFont:FONT(16.f)
     constrainedToSize:CGSizeMake(300.f, 30.f)
     lineBreakMode:UILineBreakModeWordWrap];
     
     UIImageView *handView = [[[UIImageView alloc] initWithImage:[UIImage imageNamed:@"hand.png"]] autorelease];
     handView.frame = CGRectMake(150.f + actionSize.width/2+2*MARGIN, 9.f, 13.f, 13.f);
     [eventActionBut addSubview:handView];
     }
     */
}
