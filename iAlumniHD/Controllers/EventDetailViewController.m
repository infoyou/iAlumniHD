...
#import "EventDetailActionView.h"
...
- (void)addActionForDifferentType {
    ...
    int actionType = self.event.actionType.intValue;
...
- (void)initHeadView {
    ...
    _headView = [[[EventDetailHeadView alloc] initWithFrame:CGRectMake(0, 0,
...
- (void)addBottomToolbar {
    // Action View
    _bottomToolbar = [[[EventDetailActionView alloc] initWithFrame:CGRectMake(0, self.frame.size.height - BOTTOM_TOOL_H+48.f,
                                                                              self.view.frame.size.width,
                                                                              BOTTOM_TOOL_H)
                                                             event:self.event
    ...
    [UIView animateWithDuration:0.2f
                     animations:^{
                         ...
                         _tableView.frame = CGRectMake(_tableView.frame.origin.x,
                                                       _tableView.frame.origin.y,
                                                       _tableView.frame.size.width,
                                                       _tableView.frame.size.height - BOTTOM_TOOL_H);
                     }];
}
...
- (void)adminCheckin:(id)sender {
    ...
    WXWNavigationController *mNC = [[[WXWNavigationController alloc] initWithRootViewController:mClubSearchVC] autorelease];
...
- (void)goContracts {
    if ([@"" isEqualToString:self.event.tel]) {
        return;
    }
...
- (void)displayIntro {
    ...
    [self gotoUrl:url aTitle:LocaleStringForKey(NSIntroductionTitle, nil)];
}
            ...
            switch (buttonIndex) {
                case CALL_ACTION_SHEET_IDX:
                {
                    if (self.event.tel && self.event.tel.length > 0) {
                        NSString *phoneNumber = [self.event.tel stringByReplacingOccurrencesOfString:@" " withString:@""];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    if (nil == _sectionHeaderView) {
        ...
        _eventActionButton = [[[WXWImageButton alloc] initImageButtonWithFrame:CGRectMake(_sectionHeaderView.frame.size.width, MARGIN * 2, ACTION_BUTTON_WIDTH, ACTION_BUTTON_HEIGHT)
                                                                       target:nil
                                                                       action:nil
...
- (void)triggerOnlinePayment {
    ...
    ShowAlert(self, LocaleStringForKey(NSNoteTitle, nil), LocaleStringForKey(NSPayTempTitle, nil), LocaleStringForKey(NSIKnowTitle, nil));
...
- (void)goPay:(NSData *)result {
    ...
    _paymentView.modalPresentationStyle = UIModalPresentationFullScreen;
    [[APP_DELEGATE foundationRootViewController] presentModalViewController:_paymentView animated:YES];
